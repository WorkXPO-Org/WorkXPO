...
export default function ProjectDetails() {
  ...
  if (loading)
    return (
      <div className="text-center p-10 text-text-main">
        Carregando detalhes da inovação...
      </div>
    );
  if (!project)
    return (
      ...
      <main className="container mx-auto px-5 py-10 max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div className="md:col-span-2 space-y-10 font-sans">
            <div>
              <h2 className="text-2xl font-semibold text-dark mb-4 border-b border-primary/30 pb-2">
                Sobre o Projeto
              </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="bg-white p-6 rounded-xl border border-dark/5">
                <h3 className="text-sm font-bold text-text-slate uppercase mb-2">
                  Status de Andamento
                </h3>
          ...
          <aside className="bg-dark text-text-light p-8 rounded-2xl shadow-lg h-fit space-y-6 font-sans">
            <h2 className="text-2xl mb-4 border-b border-primary/20 pb-2 text-primary">
              Informações Técnicas
            </h2>

            <div>
              <p className="text-xs text-primary font-bold uppercase tracking-tighter">
                Instituição Realizada
              </p>
              <p className="text-lg">
                {project.institution || "Não informada"}
              </p>
            </div>
              ...
              <ul className="list-disc list-inside text-sm mt-1 opacity-90">
                {project.studentsGroup?.length > 0 ? (
                  project.studentsGroup.map((student) => (
                    <li
                      key={student.id}
                      className="group list-none flex items-center"
                    >
                      {/* the name of the student is clickable */}
                      <a
            ...
            <div className="pt-4 space-y-3">
              ...
              <a
                href={`mailto:${project.contactLink}`}
